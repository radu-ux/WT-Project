...
import Logo from './static/teacher login logo.png';
...
function TeacherLogin() {
    ...
    const handleSubmit = async event => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5000/login/teacher', optionHeaders)
            const data = await response.json();
            AuthInfo.authenticateTeacher(data.userId);
            history.push('/teacher-homepage/tid/' + data.userId);
        } catch {
    ...
    return (
        <Container>
            <Card id="login-card-container" className="mx-auto" style={{height:'350px', marginTop:'10%'}}>
                ...
                <Card.Title style={{textAlign:'center', color:'rgb(30, 103, 160)'}}>Teacher Login</Card.Title>
